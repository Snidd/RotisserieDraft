...
namespace RotisserieDraft.Tests.Domain
{
	...
	public class TestPickRepository
	{
		...
		{
			IPickRepository repository = new PickRepository();
			var pick1 = repository.PickCard(_drafts[0], _members[0], _cards[0]);
			var pick2 = repository.PickCard(_drafts[0], _members[0], _cards[1]);
			var pick3 = repository.PickCard(_drafts[0], _members[0], _cards[2]);

			// use session to try to load the pick
			using (ISession session = _sessionFactory.OpenSession())
			{
				var fromDb = session.Get<Pick>(pick1.Id);
				...
				fromDb = session.Get<Pick>(pick2.Id);
				// Test that the pick was successfully inserted
				Assert.IsNotNull(fromDb);
				...
				fromDb = session.Get<Pick>(pick3.Id);
				// Test that the pick was successfully inserted
				Assert.IsNotNull(fromDb);
